 ...
 */
exports.cleanupExpiredGuessMeQueue = (0, scheduler_1.onSchedule)({
    ...
    try {
        ...
        if (expiredEntries.empty) {
            firebase_functions_1.logger.log("No expired GuessMe queue entries to clean up");
            return;
        }
        // Delete in batches
        const batchSize = 500;
        ...
        firebase_functions_1.logger.log(`Cleaned up ${totalDeleted} expired GuessMe queue entries`);
    }
    catch (error) {
        firebase_functions_1.logger.error("Error cleaning up GuessMe queue:", error);
    }
});
/**
 ...
 */
exports.expireStuckConnectionConfirmations = (0, scheduler_1.onSchedule)({
    ...
    try {
        ...
        if (stuckSessions.empty) {
            firebase_functions_1.logger.log("No stuck connection confirmations to expire");
            return;
        }
        let totalExpired = 0;
        for (const doc of stuckSessions.docs) {
            ...
            if (players && players.length >= 2) {
                const clearPromises = players.map((playerId) => admin
                    .firestore()
                    .collection("profiles")
                    .doc(playerId)
                    .update({
                    ...
                    .catch((error) => {
                    firebase_functions_1.logger.warn(`Failed to clear status for ${playerId}:`, error);
                }));
                await Promise.all(clearPromises);
            }
            totalExpired++;
        }
        firebase_functions_1.logger.log(`Expired ${totalExpired} stuck connection confirmations`);
    }
    catch (error) {
        firebase_functions_1.logger.error("Error expiring stuck connection confirmations:", error);
    }
});
/**
