...
/// Abstract repository interface for local news post interaction operations.
abstract class INewsInteractionRepository {
  /// Toggles a like on a news post. Returns `true` if now liked, `false` if unliked.
  ...
  /// Gets interaction info (like count, comment count, isLiked) for a news post.
  ...
  /// Gets paginated comments for a news post.
  Future<Either<Failure, List<Comment>>> getComments({
    required String postId,
    int limit = 20,
  });

  /// Streams real-time comments for a news post.
